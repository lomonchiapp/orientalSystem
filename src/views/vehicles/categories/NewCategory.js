...
export const NewCategory = ({open, setOpen}) => {
  ...
  const handleImageUpload = event => {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      const storageRef = ref(storage, `images/categories/${v4()}${fileExtension}`)
    ...
    <form>
      ...
      >
        <Grid item xs={12}>
          ...
          <Typography variant='body2'>Aquí puedes agregar una nueva categoría.</Typography>
        </Grid>
        <Grid item xs={7}>
        ...
        <Grid item xs={5}>
          <FormControl>
            <Button
              ...
              sx={{ my: 4, height: 56 }}
            >
              {image ? 'Imagen Subida' : 'Subir Imagen'}
        ...
        <Grid item xs={12}>
          <Button variant='contained' color='primary' onClick={createCategory}>
            ...
            <span className='align-middle ml-50'>Agregar Categoría</span>
          </Button>
        </Grid>
      </Grid>
    </form>
